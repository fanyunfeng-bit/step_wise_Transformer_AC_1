...
class AttentionModel(nn.Module):
    ...
    def forward(self, input, mask, mask_first, state, i):
        """
        ...
        _log_p, value = self._inner(input, mask, mask_first, state, i)
        # print('_log_p: ', _log_p, _log_p.shape)
    ...
    def _inner(self, input, mask, mask_first, state, i):
        ...
        if self.is_vrp:
            ...
            graph_size = state.loc.shape[1]

        # # Perform decoding steps
        # i = 0

        # if self.is_vrp:
        #     mask = torch.zeros(batch_size, 1, graph_size)
        # else:
        #     mask = torch.zeros(batch_size, 1, graph_size)
        ...
        if self.is_vrp:
            ...
            mask_tmp = (~mask).view(batch_size, graph_size+1, 1).repeat(1, 1, self.embedding_dim).float()
            ...
            if i > 0:
                mask_tmp = mask ^ mask_first  # 异或
                # print('mask_first: ', mask_first)
                embeddings, init_context = self.embedder(input, mask=mask_tmp)
                mask_tmp = (~mask_tmp).view(batch_size, graph_size, 1).repeat(1, 1, self.embedding_dim).float()
                init_context = torch.sum(embeddings * mask_tmp, dim=1) / (graph_size + 1 - i)  # 计算graph_embedding时也将被选择过的mask掉
        ...
        log_p, value, mask = self._get_log_p(fixed, state)
        ...
        # # 原来写法并没有做到在tsp问题下第一个选中的node一直不被mask，因此添加以下代码
        # mask = state.get_mask()
        # if not self.is_vrp and i == 0:
        #     mask_first = mask
    ...
    def _get_log_p(self, fixed, state, normalize=True):
        ...
        # print('shape: ', self.project_step_context(self._get_parallel_step_context(fixed.node_embeddings, state)).shape)
        ...
        # critic: value
        value = self.value_head(query)  # query已经包含了当前的所有信息：（剩余的node，当前node）

        # Compute logits (unnormalized log_p)
        # decoder 的mask体现在计算compatibility和logits时将不能被选择的node对应的值设为-math.inf，即概率为0
