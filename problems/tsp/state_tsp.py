...
class StateTSP(NamedTuple):
    ...
    def update(self, selected):
        ...
        if self.cur_coord is not None:  # Don't add length for first action (selection of start node)
            ...
            if self.i.item() == self.loc.size(-2)-1:
                # 若选择的是最后一个点，则reward不仅是增加的距离，还包括从最后一个回到第一个的距离
                reward = -(cur_coord - self.cur_coord).norm(p=2, dim=-1) - \
                         (cur_coord - self.loc[self.ids, self.first_a]).norm(p=2, dim=-1)
            else:
                reward = -(cur_coord - self.cur_coord).norm(p=2, dim=-1)
        else:
            reward = torch.zeros(batch_size, 1)  # 第一个动作没有reward，只是选择了起始点
        ...
        return self._replace(first_a=first_a, prev_a=prev_a, visited_=visited_,
                             lengths=lengths, cur_coord=cur_coord, i=self.i + 1), reward
